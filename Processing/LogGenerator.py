...
def GenerateLog(path,orientation,rotationAngle):

    takeClosest = lambda num,collection:min(collection,key=lambda x:abs(x-num))
    # Setup:
    pipeline = rs.pipeline()
    cfg = rs.config()
    cfg.enable_device_from_file("{}{}.bag".format(path,orientation), True)
    ...
    try:
        ...
        #while time.time() < t_end:
            ...
            if (color_timestamp_str not in all_color_timestamps):
                ...
                rotated = ndimage.rotate(color_image, rotationAngle)

                # writing the png to dir
                dirPath = path + orientation.lower() + "_frames"
                try:
                    os.mkdir(dirPath)
                except OSError:
                    pass
                cv2.imwrite(os.path.join(dirPath, "{}.png".format(color_timestamp_str)), rotated)
        ...
        with open(os.path.join(path,orientation.lower() + '_log.json'), 'w') as f:
            json.dump(timestampsDepthJson, f)
